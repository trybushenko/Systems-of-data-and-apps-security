...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Лабораторна робота No1")
    ...
    def call_wind3(self):
        self.get_a2, self.if_get_a2_in = QInputDialog.getText(self, "ChooseUser",
                                                              "Ви новий користувач? (так - 1, ні - 0):")
        if self.get_a2 == '1':
            file = open('logbook.txt', 'w')
            self.input_name()
            file.writelines(self.get_a4)
            index = str(self.get_a4).find(',')
            right_pass = self.right_password(str(self.get_a4)[index:])
            ...
            while True:
                ...
                self.get_a5, self.if_get_a5_in = QInputDialog.getText(self, "Question",
                                                                      "Розрахуйте секретну фенкцію. Вхідні дані: {0}, "
                                                                      "{1}".format(
                                                                          a, x))
                count = 0
                ...
                if logbook[i] == self.get_a4:
                    ...
                    while True:
                        ...
                        self.get_a6, self.if_get_a6_in = QInputDialog.getText(self, "Question",
                                                                              "Розрахуйте секретну фенкцію. Вхідні "
                                                                              "дані: {0}, {1}".format(
                                                                                  a, x))
                        count = 0
                        while self.get_a4 != math.lg(a + x):
